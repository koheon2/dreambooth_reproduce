...
class DreamBoothMultiSubjectDataset(Dataset):
    def __init__(self, subjects_config, tokenizer, size, center_crop, tokenizer_max_length, with_prior_preservation, use_augmentation=False):
        ...
        for subject in subjects_config:
            ...
            if not os.path.isdir(instance_data_dir):
                print(f"Warning: Instance data directory not found for subject '{subject['name']}': {instance_data_dir}")
                continue
            
            subject_instance_paths = sorted([
                os.path.join(instance_data_dir, f)
                for f in os.listdir(instance_data_dir)
                if f.lower().endswith((".jpg", ".jpeg", ".png", ".webp"))
            ])
            self.instance_images_paths.extend(subject_instance_paths)
            self.instance_prompts.extend([subject["instance_prompt"]] * len(subject_instance_paths))

            if self.with_prior_preservation and "class_data_dir" in subject and "class_prompt" in subject:
                class_data_dir = subject["class_data_dir"]
                if not os.path.isdir(class_data_dir):
                    print(f"Warning: Class data directory not found for subject '{subject['name']}': {class_data_dir}")
                else:
                    subject_class_paths = sorted([
                        os.path.join(class_data_dir, f)
                        for f in os.listdir(class_data_dir)
                        if f.lower().endswith((".jpg", ".jpeg", ".png", ".webp"))
                    ])
                    if not subject_class_paths and subject.get("num_class_images", 0) > 0:
                        print(f"Warning: Class data directory for '{subject['name']}' is empty but num_class_images > 0. Ensure class images are generated.")

                    self.class_images_paths.extend(subject_class_paths)
                    self.class_prompts.extend([subject["class_prompt"]] * len(subject_class_paths))
        
        if not self.instance_images_paths:
            raise ValueError("No instance images found across all subjects. Check instance_data_dir paths.")
        ...
        if self.use_augmentation:
            print("Applying data augmentation: RandomHorizontalFlip")
    ...
    def __getitem__(self, index):
        ...
        instance_image_path = self.instance_images_paths[instance_idx]
        ...
        try:
            ...
            raise RuntimeError(f"Could not load instance image: {instance_image_path}") from e
        ...
        if self.with_prior_preservation and self.num_class_images > 0:
            ...
            try:
                ...
                raise RuntimeError(f"Could not load class image: {class_image_path}") from e
...
def collate_fn_dreambooth(examples, with_prior_preservation):
    ...
    if with_prior_preservation and "class_pixel_values" in examples[0]: # Check if class data is present
...
def ensure_class_images(subject_config, sd_model_name, scheduler_type, device, dtype, seed, inference_steps):
    ...
    try:
        pipe = StableDiffusionPipeline.from_pretrained(
            ...
            scheduler = get_scheduler(scheduler_type, sd_model_name)
        ).to(device, dtype)
    except Exception as e:
