...
class DreamBoothDataset(Dataset):
    def __init__(self, instance_data_root, instance_prompt, tokenizer, resolution=512, center_crop=False,
                class_data_root=None, class_prompt=None, num_class_images=0, ablation_type="none",
                class_noun_pool=None, real_class_noun=None,):
        ...
        self.real_class_noun = real_class_noun
        self.ablation_type   = ablation_type   
        self.class_noun_pool = class_noun_pool
    ...
    def __getitem__(self, idx):
        ...
        if self.has_prior:
            ...
            if self.ablation_type == "no_class_noun":
            # an empty prompt (or you could use e.g. "a photo of") 
                prompt_text = ""
            elif self.ablation_type == "random_class_noun":
                # sample a wrong noun different from the real one
                import random
                pool = [n for n in self.class_noun_pool
                        if n != self.real_class_noun]
                wrong = random.choice(pool)
                prompt_text = f"a photo of {wrong}"
            else:
                # normal prior-pres prompt
                prompt_text = self.class_prompt

            # tokenize that ablated prompt_text
...
def main():
    ...
    parser.add_argument("--ablation_type", type=str,
        choices=["none","no_class_noun","random_class_noun"],
        default=None,
        help="Sec 4.3 class-prior ablation mode"
    )
    parser.add_argument("--class_noun_pool", nargs="+",
        default=None,
        help="list of class nouns for random_class_noun"
    )
    ...
    ablation_type   = args.ablation_type   or base_cfg.get("ablation_type", "none")
    class_noun_pool = args.class_noun_pool or base_cfg.get("class_noun_pool", [])
    ...
    train_dataset = DreamBoothDataset(
        instance_data_root = base_cfg["instance_data_dir"],
        instance_prompt    = base_cfg["instance_prompt"],
        tokenizer          = tokenizer,
        resolution         = base_cfg.get("resolution",512),
        center_crop        = base_cfg.get("center_crop", False),
        class_data_root    = base_cfg.get("class_data_dir"),
        class_prompt       = base_cfg.get("class_prompt"),
        num_class_images   = base_cfg.get("num_class_images",0),
        ablation_type      = ablation_type,
        class_noun_pool    = class_noun_pool,
        real_class_noun    = base_cfg.get("class_prompt","").split()[-1]
    )
